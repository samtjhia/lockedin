-- Leaderboard: count at most one active/paused session per user so duplicate sessions don't double-count time.

-- 1. Live leaderboard (daily/weekly)
...
BEGIN
  ...
  completed_sum AS (
    SELECT s.user_id, SUM(s.duration_seconds)::bigint AS sec
    FROM sessions s
    WHERE s.started_at >= start_time
      AND s.mode NOT IN ('short-break', 'long-break')
...
-- 2. Leaderboard for a specific date (Toronto day)
...
-- 3. Leaderboard for a specific week (Monday 00:00 Toronto)
